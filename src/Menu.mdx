...
**Do not use it on a menu that has a `MenuSub` yet.** Measured: once the
pointer has been inside a submenu panel, neither the submenu nor the menu
closes again. The parent's close is driven by hover on its trigger, and the
pointer never returns there. Leaving *without* entering a submenu closes
correctly. This is Base UI's own hover choreography, so there is nothing of
ours to fix; `PLAN.md` §6.2 carries the measurement.
