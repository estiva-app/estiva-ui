 ...
 *   frame's first row (Ship's, 2026-09-02, verbatim; its product title
 *   moved to the caller).
 ...
 *   catch clicks, so the content underneath stays reachable (Peek's,
 *   2026-09-02, verbatim).
 *
 * Geometry both apps agreed on before extraction: 52px tall, `pl-5
 * pr-[26px]`, right cluster `gap-[6px]`. The left region is two slots
 * side by side — `menu` (the burger, in an app whose navigation
 * collapses) and `brand` (the logo or name beside it; Katerina,
...
export interface TopBarProps {
  ...
  /** Far left: the burger, in an app whose navigation collapses. Stays the caller's — the bar only places it. */
  menu?: ReactNode
  /** Beside the burger: the app's name or logo mark. */
  brand?: ReactNode
  /** The centre: a search field, or a launcher affordance. */
  search?: ReactNode
...
export function TopBar({ variant = 'solid', menu, brand, search, right, className }: TopBarProps) {
    ...
    >
      {(menu || brand) && (
        <div className={cn('flex shrink-0 items-center gap-2', floating && 'pointer-events-auto')}>
          {menu}
          {brand && <div className="flex items-center text-body-2-strong text-text-primary">{brand}</div>}
        </div>
      )}
