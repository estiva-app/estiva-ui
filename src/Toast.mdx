...
- `neutral` states, `success` celebrates, `brand` speaks with the
  accent's voice, and `warning` says something needs knowing — a notice
  usually paired with `durationMs: 0` and a Dismiss, because a warning that
  fades takes itself with it.
