...
const meta = {
  title: 'Frame/NavItem',
  component: NavItem,
  args: { label: 'Documents', href: '#', active: false },
...
export const WithIconAndCount: Story = {
  args: { icon: <IconListDetails size={16} stroke={1.5} />, count: 18, countLabel: '18 open' },
}

/** A zero is not drawn at all — this row's rule (a tab draws its zero; both are deliberate). */
export const ZeroDrawsNothing: Story = {
  args: { icon: <IconBox size={16} stroke={1.5} />, count: 0, countLabel: '0 open' },
}

/** The label gives way; the count never does. */
export const LongLabelTruncates: Story = {
  args: { label: 'A name that runs much longer than the column has room for', count: 7, countLabel: '7 open' },
}
