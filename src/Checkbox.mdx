import { Meta, Canvas, Controls } from '@storybook/addon-docs/blocks'
import * as CheckboxStories from './Checkbox.stories'

<Meta of={CheckboxStories} />

# Checkbox

A 16px square that fills with the accent when checked. Controlled only —
the parent owns the state.

<Canvas of={CheckboxStories.Toggles} />

## When

- A yes/no the person sets: an option in a form, an include/exclude in a
  list.
- Inside a clickable row: pass **no `onChange`**. The square then only
  draws the state: it is hidden from assistive technology and takes no
  focus, so the row is one control. The row says the state itself —
  `aria-pressed` on a button row, `aria-selected` on an option in a list.

<Canvas of={CheckboxStories.InsideARow} />

- With words beside it: pass **`label`**. The words are part of the
  target — clicking them toggles the box — and they name it, so no
  `aria-label` is needed.

<Canvas of={CheckboxStories.WithLabel} />

## When not

- One choice out of several → **Select** (a value) or **Tabs** (a view).
- Inside a **Menu** — a checkbox inside a `menuitem` is invalid HTML;
  a checkable list lives in a DialogShell.

## How

```tsx
import { Checkbox } from '@estiva-app/ui'

<Checkbox checked={done} onChange={setDone} aria-label="Mark as done" />
```

- Built on Base UI Checkbox, as Base UI renders it: a `<span>` with
  `role="checkbox"` and `aria-checked`, and a hidden `<input>` beside it
  for forms. Its click never reaches the row around it.
- Give it an `aria-label`, or words of its own with `label`. Words written
  beside it by hand name nothing and toggle nothing; `label` does both.

```tsx
<Checkbox checked={on} onChange={setOn} label="Label" />
```

- With `label` it draws an enclosing `<label>`: the box, 8px, then the
  words in `body-2`. `className` stays on the box. When disabled, only the
  box shows it; the words keep their colour and lose the pointer.
- It does not move when it toggles: the tick is always in the box, hidden
  when unchecked, so both states hang on a line of text the same way.

## Keys

| Key | Does |
|---|---|
| Tab | Onto the box. |
| Space | Toggles it. |
| Enter | Nothing. As on a native checkbox, Enter is the form's key. |

## Props

<Controls of={CheckboxStories.Unchecked} />
