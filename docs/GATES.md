...
| ✅ **UIG-12** | estiva-ui branch `gates/12-registry`: `registry.json` (81 entries, **352 props**, schema 1), the builder and the schema in `src/registry/`, `estiva-ui find` as a bin of the package, and `registry:check` in CI. 81 names over 53 files = 74 components + 6 helpers + 1 hook; 53 take their line from their own page, 28 from the comment above them. Every docs and story id was checked against a real `storybook build`. `gates:status` reads 7 of 7. See **UIG-12: building it**, below. |
