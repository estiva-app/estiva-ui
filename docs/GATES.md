...
| ✅ **UIG-12** | estiva-ui branch `gates/12-registry`: `registry.json` (81 entries, **374 props**, schema 1), the builder and the schema in `src/registry/`, `estiva-ui find` as a bin of the package, and `registry:check` in CI. 81 names over 53 files = 74 components + 6 helpers + 1 hook; 53 take their line from their own page, 28 from the comment above them. Every docs and story id was checked against a real `storybook build`. `gates:status` reads 7 of 7. See **UIG-12: building it**, below. |
...
| **now** | **UIG-13** (widen the registry to Peek and Ship), which UIG-20 and UIG-25 both wait on. **UIG-10, reopened**: the `platform` helper in estiva-foundation, then the command. **UIG-11** (Leaf, made with `create-estiva-app`) waits for that release, and needs Jan for sign-in on the real Estiva ID and for its event kinds. UIG-14 to UIG-16 (estiva-ui's usage rules) can run beside either. UIG-30 any time. UIG-31 much later |
