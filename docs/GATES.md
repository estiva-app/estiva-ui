...
| **now** | UIG-19: **re-scoped 23 September** to "Lock the contract in CI". No shared Storybook; the search is `estiva-ui find`, and running it unasked is UIG-20's (§0). **UIG-11** (Leaf) joins both when it exists |
...
| UIG-19 | Lock the contract in CI (re-scoped 23 September) | estiva-ui | peek, ship | a CI step runs an `npm run …contract…` script; neither app keeps its own copy of the section check |
