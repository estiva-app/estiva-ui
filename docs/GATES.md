...
| ⬜ **UIG-26** | The close. Waits on UIG-11 (Leaf, held) and on Katerina's rulings from the audit before UIG-26 (§0, 25 September), which found its scope and blockers out of date (F1–F4). `gates:status` 0 of 1. |
...
| ⬜ **UIG-38** | New, 23 September, found in UIG-21: the gate misses a hand-drawn `<table>` and an Escape handled by hand on one element (T11's "no keydown listener" reads only page-wide listeners). Two questions wait on Katerina. In `gates:status` since 25 September, 0 of 1. |
...
| ⏸ next | Katerina's rulings on the audit's other findings (groups B, C, D, F), then UIG-26 rewritten from them |
...
**⚠️ GitHub stopped running Actions for the organisation on 24 September, about 22:00 UTC**: "The job was not started because recent account payments have failed or your spending limit needs to be increased." Ship's deploy ran first; Peek's deploy (#361), and every job on peek #362 and ship #194, were refused. The merged Peek change draws nothing differently. Nothing can merge into any repo until the billing is sorted, because `gate` is required and cannot run.

**Waiting on her rulings:** B1–B4, B6–B15 (B7 is UIG-38's two questions), C1–C9 (changes made without her ruling), D1–D13 (a home for what would be lost at the close), E1 (the Ship project description, which she pastes), E4, and F1–F4 (UIG-26's scope, its deleted baseline, Leaf, and UIG-30 in every app's status).
