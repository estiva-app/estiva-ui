...
**23 September 2026. UIG-17 is built and waiting on Katerina: every reusable part of Peek carries a written rule beside it, and its Storybook was rebuilt to one principle.** 32 commits on `gates/17-usage-peek`, nothing pushed, no PR — she merges. `gates:status` in peek reads UIG-17 **4 of 4**.

**The numbers were eleven days old here too.** The ticket says 115 parts; `GATES.md` said 53 reusable. Rebuilt from the branch's own code, after `main` merged in: **144 parts in 117 files — 20 re-export, 48 reusable, 76 one-off, 0 promote-candidates.** All 48 have a page. The one candidate the app had, `EditedMarker`, **she ruled stays Peek's**, so the promote list closes with her word rather than an inference; the classification is written into the component's own comment.
...
**Also merged in, rather than left for PR day:** 19 commits of `main`, whose one conflict was in the file the branch had restructured — `FoldersPage.tsx`, where FOL-41 rewrote the Folders header while the branch was splitting the page into a view and a page. FOL-41 was built into the split, and the page's stories followed it. A near miss worth recording: `main` edited `FolderTree.stories.tsx` while the branch had renamed it; git followed the rename.

**Left:** the PR, and an `npm install` in peek — the shared `node_modules` holds `@estiva-app/interop` 0.24.0 while `main` now asks for ^0.27.0, which is the only reason two type errors appear locally. Next: UIG-19.
