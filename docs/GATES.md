...
**26 September 2026. 0.36.0 released, and both apps took it.** estiva-ui PR #99 (merge `9d0b61e`, tag `v0.36.0`, npm latest), peek PR #372 (`41dba82`) and ship PR #198 (`788c49c`), all merged and deployed; every job green on each. Before merging she answered the five questions on the review page (`https://claude.ai/artifact/6XfmgaRLjtXH71xqSonEkC`): **Q1 A** Peek UnreadDot says "No alternative: Peek draws no unread count"; **Q2 A** EmptyState names SkeletonList and SkeletonBar; **Q3 A** ScrollArea's When not also names ListColumn; **Q4 A** "No alternative:" is its own line under the bullets; **Q5 B** PersonRow's page gains the line "Always pass `type`", and the code stays as it is. In the apps: every escape named (Peek 20 + her C4 record note, Ship 8); `gate` is `npx estiva-gates ci`; the When not words on 20 Peek and 5 Ship pages; Peek's search field keeps its hand-made box with her reason, PersonRow's and Ship's DescriptionEditor's reasons cover the click (`no-raw-element` escapes: Peek 0 → 2, Ship 0 → 1); MembersDialog, ProjectTickets, RouterLink and ThreadPanelLoading parked under Peek's "To be removed"; both debt pages point at `gates:status -- --escapes` and name the folders the gate skips (Peek: `demo-scenarios`; Ship: none). UIG-26's text in Ship is rewritten by name. **Also asked the same day:** our scrollbar should show wherever content is larger. Measured: every column already scrolls on our `ScrollArea`, but its bar stayed at opacity 0 until the pointer was over it (a 40-folder list in the File page's side column: 1,386px in a 442px box, no bar). estiva-ui PR #100 (0.36.1) shows it at rest whenever there is more to scroll to; it waits for her look at the pictures.

**25 September 2026, evening. Katerina's rulings on both reviews, built.** GitHub runs again; peek #362 #364 #367 and ship #194 #195 merged and deployed, estiva-ui 0.35.2 out. Her rulings, and what each became (estiva-ui **0.36.0**, branch `gates/rulings-0925`; the apps take it after):
- **Escapes (B1, B3; UIG-38's Escape):** every escape is listed from the code with its age from git, and `gates:status` shows the ones older than 30 days, to keep with a new reason or fix; `-- --escapes` shows all. A marker names the rules it keeps off, `// @estiva-escape(<rule>): <reason>`; an unnamed one is an error, and `estiva-gates name-escapes` names them once (20 in the package). An Escape handled by hand on one element is **allowed**; tables wait (UIG-38).
- **The wall:** the editor hook runs the token contract (B4) and hands Claude a copied look as a note (R10). `style` on a part passes placement only (B6); a style with quoted keys or kept in a `const` is read (R9). A clickable plain element is refused, naming `Button` or `Card` with `href` (R5): on main it finds three, two already escaped (Peek PersonRow, Ship DescriptionEditor) and Peek's search field, which she ruled stays hand-made (card 2, B). `estiva-gates ci` is the `gate` job in every repo (R13), and adds the count committed (B2) and the wall's own settings: the hook refuses a raw button, every skipped folder is named on the debt page (B11, B13b). A branch must be up to date with main to merge (B13a): **on in estiva-ui** (ruleset `gate on main`, 25 September); in Peek and Ship her account is no longer an admin (`gh api` 404, `permissions.admin: false`), so an admin switches it on there — `strict_required_status_checks_policy: true` in the same ruleset.
- **The catalogue:** When not names the part to use instead, or starts a line "No alternative:" (B8; the words are hers to approve first, 29 pages). What a part owns is worked out from the code, and app parts inherit it (B9). A reusable part links to its page by the page's title; one with no link fails, a one-off needs none (B10, R22). `gates:status` in an app names the unused parts for her to rule on (R20). Every story is drawn in the package's gate with axe off (R19; apps later). The code under every How compiles, in the gate (R18: 68 blocks, 0 real mistakes; seven stand-in names allowed; ChipInput's page shows its option type). An unclassified part is proved to fail (C6).
- **Recorded as limits, not built:** B12 (a header moved into a component of its own), R12. **Not needed:** B14 (Jan and the rule list). **Later:** B15 (Chromatic), R21 (the skill's Gotchas, at UIG-26). **Group C:** she accepted the twelve changes made without her ruling, and asked for the two: C5 (D3's line on PersonRow's page, drafted for her) and C6 (the test above).
- **The leftovers (group D), one by one:** 1 A → UIG-40; 2 B; 3 B; 4 A (settled: a card that is a link is built on Link, her UIG-27 ruling); 5 B; 6 A and 7 A → UIG-41; 8: the unused parts go under Peek's "To be removed", with the date; 9 later, hers; 10 B; 11 A (settled by EmptyState's own page: a failure in words, never red); 12 later; 13 A and 14 A → UIG-42; 15 B; 16 B; 17 B; 18a B; 18b B; 18c A; 18d B; 19 A (no ticket; mention to Jan with the billing note).
- **UIG-26 (F1–F3):** it waits for every other ticket and for Leaf, which stays held; its "UIG-10 made app" baseline, deleted on 17 September, is remade from 0.21.0 and 0.23.0 when it runs. Its text is rewritten by name.
- **One slip, told to her:** the first local run of the draw-only Storybook project ran axe on every story, because the addon's own global beat the switch I set. The switch is now the story parameter, proved by a run with axe replaced by a trap.

**25 September 2026, morning. The second review, and what it fixed.** Five read-only checkers went through the project again from scratch, without the first audit's list: **30 new findings, R1–R30**, on a review page of their own (`https://claude.ai/artifact/8aTHx22Z5PMAvN1KD7QRtF`). Four were broken and are fixed: a made app's required `gate` failed on its first commit (its Storybook had no Docs pages, so `registry:check` refused a link; R1); estiva-ui's own hook still failed open (R2); `gates:compare` had crashed since UIG-19 (R3); and status's hook check could pass without reading the button (R4). **Released as 0.35.1** (estiva-ui PR #97, merge `e727ba3`, tag `v0.35.1`); the compare fix and this record's previous update merged as PR #96 (`9bdedc9`). **Then, by her order, the fixes that need no ruling** (estiva-ui PR #98, released as 0.35.2): the gate and the hook read every script type in an app's `src` (R6); `find` notes a neighbour whose checkout is behind main (R15); the Storybook-map check reads Storybook's own index, both ways of writing a heading order and both Introduction styles, and runs in the package too (R16); "Choosing a component" names every part with a page, held by a test (R17); an app page's code must sit under How itself (R18, first half); `estiva-ui check` refuses a usage page that names a part or a story that is not there (R14) — measured first: across both apps it found the stale names FOL-23 left and nothing else. The apps' halves wait: **peek #364** (seven pages that named deleted parts and stories) and **ship #195** (its heading order and Introduction, and one old title); they cannot merge until GitHub runs again (below). **Not done, and why:** compiling every How block in CI (R18's second half) — a prototype failed 59 of 68 blocks, nearly all on placeholder names like `person`, so it needs a design, not a patch. **GitHub, found on the way:** only the private repos are refused. The organisation is on GitHub's Team plan, 3,000 Actions minutes a month for private repos; Peek and Ship used 4,877 in September (923 runs; August 350). estiva-ui, estiva-docs and estiva-foundation are public and run free. Only the organisation's owner can change its billing. **Also recorded:** estiva-ui's `a11y` job, run on main after each merge, fails three stories (FilePicker's two, SectionLabel's Muted); accessibility is handled later, all at once (her ruling), so it is left.

**25 September 2026, night. The audit before UIG-26, and what it fixed.** Before the last ticket, Katerina asked for a thorough check of every ticket against the guide she agreed to (the artifact *Estiva's Four Gates*, committed as `GATES-GUIDE.md`). Read-only checkers compared Plan B, tactic by tactic, with what is on `main` in the three repos, and every ticket as first written on 12 September with its text today. **52 findings**, on a review page in six groups (`https://claude.ai/artifact/7kqeDgG13Rcun9X6ccZxpD`): 5 broken now, 16 plan promises not fully kept, 9 changes made without her ruling, 13 things that would be lost at the close, 5 records out of date, and 4 about UIG-26 itself. **The wall holds:** 0 errors in Peek (310 files) and Ship (156), escape counts equal to the committed count files, peek #192's own Folders page refused by today's hook, "gate on main" in all three with no bypass. **Her ruling, 25 September: "fix the broken things, merge them", then review again from scratch; the other questions later.** Fixed and merged: **estiva-ui PR #95, released as 0.35.0** (merge `35d6cc4`, tag `v0.35.0`) — Ship read at its top folder after PER-19 moved it out of `web/` (A1); the hook fails closed, and `gates:status` runs it on a raw `<button>` and passes only on exit 2 (A2); Peek's Storybook-map checks moved into `estiva-ui check`, reading the catalogue in memory, not a gitignored `registry.json` (A3, A5); every repo line in `gates:status` fetches and says how far the checkout is behind main (A4); a made app's `gate` runs the token lint, and a test runs every app check on a freshly made app (B5); the made `CLAUDE.md` points at the skill for the rule list (B16). **peek PR #361** (`76d570b`): the Introduction and `HEADINGS` lose the deleted Huddles heading, the count 49 → 41, two "Seen in" lines no longer name deleted stories, Peek's three copied checks go. **ship PR #193** (`ce0e37d`): takes 0.35.0; deployed. **Peek's deploy did not run: GitHub refused to start the job** ("recent account payments have failed or your spending limit needs to be increased") — the merged change draws nothing differently; it deploys once the billing is sorted. Two more of the same drift, found by the fixed status itself: Peek's status read `TopicsPage.tsx`, which FOL-23 deleted (peek PR #362), and Ship's UIG-18 check read a local `registry.json` (ship PR #194). `gates:status` over the merged mains **with peek #362 and ship #194** (open until GitHub runs again): **31 done, 8 not started, 0 could not check**; without them three Peek checks and one Ship check cannot read. The rest of the findings wait on her rulings. See **The audit before UIG-26**, below.

**24 September 2026. UIG-25 is done: a look copied by hand is warned about, and the copies found are fixed or recorded.** estiva-ui PR #93 (0.34.0, merge `ab59283`, tag `v0.34.0`, npm latest), peek PR #353 (`4689a00`) and ship PR #188 (`3047e59`), both deployed, and this record's PR. **The rule, `estiva/no-copied-look`, warns and never blocks** (Katerina, 13 September); `strict` makes it an error. **The line, her ruling of 24 September:** a class list is a copy of a part's look when it shares **4 or more look words** with it, covering **80% of the shorter list**; placement words (space, size, flex and grid, position — `no-restyled-part`'s PLACEMENT) never count. 3 words found 161 places, mostly noise; 5 missed a Card frame. **Where looks come from, never a kept list:** in an app, the installed package's parts, whose looks now travel in `registry.json` (`looks`), and the app's own exported components, read from its `src`; a part's look includes a shared look it imports; an exported `…_CLASSES` constant is a look of its own. Tested: a part added to the catalogue is covered without touching the rule. **Her rulings, from photos before and after:** the package's shared looks written once in `src/looks.ts` (A1, A2, A4, A5, N1); story frames drawn with `Card`/`MenuPanel` (B2, N2); Peek's `MetaText` (nine places, C1), `RepliesRow` (C2) and `WRITING_BOX_CLASSES` (C3); DateDivider on `SectionLabel`, which gained `tone="muted"` and `truncate` (B1: the date line 22px → 24px, accepted); Ship's issues table in a `Card` with no fill. **CommandPalette left alone** (her word). 58 before-and-after photos identical to the pixel; the two that change are hers. **Everything else she ruled record only or leave alone carries its reason in the code** (her yes, 24 September), so a warning always means a new copy: every gate reads **no-copied-look 0 warnings**. Peek and Ship copying each other is not seen by the lint (each app knows only itself and the package); the one found (D1, the comment-anchor note) is recorded. `gates:status` on the merged code: **UIG-25 6 of 6**. See **UIG-25: building it**, below.

**24 September 2026. UIG-24 is done: the browser's tooltip is refused.** estiva-ui PR #91 (0.33.0, merge `0d2a708`, tag `v0.33.0`, npm latest), peek PR #349 (`85446a2`) and ship PR #185 (`c61f1e2`), both deployed, and this record's PR. **The rule, `estiva/no-native-title`, refuses a `title` attribute** — the browser's grey hover box, not our look, not our 300ms, never on a touch screen — and names `WithTooltip` and its page. It reads the attribute, never the page (a tooltip's words are not on the page until it opens). It fires on a plain element, and on a part of the package, or an app's wrapper that hands it all its props, whose `title` is not its own: `<Button title>` reaches the `<button>`, which is the apps' status probe. In `recommended` and in the package's own set; 23 cases pin it. **The count, over the three repos in full, stories in, tests out: 0, 0 and 0.** UIG-1's six (`Reference.tsx`, three in each app) went in UIG-14, and the Folders conversation's timestamp in peek `3dc663b`. **On older commits it finds every one:** the timestamp at `3dc663b~1` (`ForeignConversationView.tsx:106`), the six at UIG-1's commits, and four nobody had counted, all real and all since gone: a mention chip's `title` (`MentionChip.tsx:53`) and three lines of the Signal-theme story page. No false positives. **The exclusions, each a decision and each a test:** see **UIG-24: building it**. **Also:** UIG-9's status check pinned the apps' rules to the three, then `no-handmade-*`; it now lets any fingerprint rule follow them, so the next one does not turn it red. Katerina, 24 September: "begin", then "yes merge and finish". `gates:status` on the merged code: **UIG-24 5 of 5**. See **UIG-24: building it**, below.

**24 September 2026. UIG-23 is done: an empty state drawn by hand is refused.** estiva-ui PR #89 (0.32.0, merge `ed261a0`, tag `v0.32.0`, npm latest), peek PR #346 (`db5149a`) and ship PR #183 (`ca367d9`), both deployed, and this record's PR. **The rule, `estiva/no-handmade-empty-state`, refuses two shapes, each naming the scope to use.** *A line drawn by hand:* a lowercase box whose only content is words (120 characters at most; a name may sit in the sentence) that shows where a list's rows would be when the list is empty — one side of a condition whose other side **is** the list (a `.map`, or a box holding only one), or under a condition that reads "empty" (`rows.length === 0`, `!rows.length`, `isEmpty(rows)`; `rows.length > 0` is the full side and passes), up to three single-child boxes around it. *A page state that cannot centre:* a `page`-scope `EmptyState` (the part itself, or a wrapper that hands everything on) inside a `ScrollArea` whose `contentClassName` has no `min-h-full` or `h-full`, drawn there directly or by one of the app's own components, followed into its file four deep; a component that only hands its `className` to one (Ship's `Activity`) is followed, not taken for the part. It is in `recommended` and in the package's own set. **It reads where a line sits, not its words** (Katerina, 24 September: "we don't need to start with No or Nothing. Can be anything"): UIG-1's wording pass fired on a hint and a status note, and on today's code it missed four of Peek's dialogs ("There is nowhere else to move it…", "Every topic is already in your Open work."). 34 cases pin it. **The count, over the three repos in full, stories in, tests out:** the package 2 (the `FilePicker` and `ReactionPicker` stories), fixed; Peek 4 (`AddToOpenWorkDialog`, `MoveFileDialog`, `MoveUnderDialog`, `StartTopicDialog`), fixed; Ship 0; wrong-scope 0 in all three. **Total 6, all fixed, 0 escaped.** False positives: none. **Her rulings, 24 September, from before-and-after photos:** the four dialog lines become `EmptyState scope="section"` (12px muted, as Ship's New issue says "Create a project first."), and Add to Open work's line sits in the rows' own `px-3` (ruling 21), which moved it from 12px left of the title to level with it (measured 0px); the two stories too; the launcher's replies and the debug line stay (below). `.verify-empty-scopes.mjs` is retired. Found on the way and fixed in peek PR #346: Peek's Introduction said 48 reusable parts; FOL-26 (#337) made `ThreadPanelLoading` a one-off, so the catalogue builds 47, and a checkout with an older `registry.json` still read 48. `gates:status` on the merged code: **UIG-23 5 of 5**. See **UIG-23: building it**, below.

**24 September 2026. UIG-22 is done: a column's header bar drawn by hand is refused.** estiva-ui PR #86 (0.31.0, merge `a98aade`, tag `v0.31.0`, npm latest), peek PR #338 (`55ccd6c`) and ship PR #178 (`52503e6`), both deployed, and this record's PR. **The rule, `estiva/no-handmade-header`, reads a shape, not a class list:** a row first in a column, with side padding of its own and a title in it (text, a value, a heading, a part that draws text, or a part's `Title`), where the column fills its height or the row has a header bar's height or hairline. It names `ContainerHeader`. It is in `recommended` and in the package's own set. 18 cases pin it, among them both Folders panes at the commit before peek `3dc663b` and the same shape in other class names. **The count, over both apps and the package in full:** Peek 3, fixed (`FilesPanel`, `ThreadPanel` twice, loading included); Ship 0; the package 2, `DialogShell` fixed (it draws `ContainerHeader` now, with its own 18px title; 8 stories in both themes photograph identical) and the command palette's level row escaped with its reason (it is the palette's input bar, the twin of its search row). **False positives: none in the apps.** Left alone on purpose, Katerina's ruling: Ship's two status-group headings (`IssueList`, `Board`) and Peek's "Huddle" card strip. **Her rulings, 23–24 September:** the part's own title, so "Files" and "Replies" read like every other column (a), with the actions 4px apart; check the package as well ("my review isn't enough"); leave the three look-alikes. `gates:status` on the merged code: **UIG-22 6 of 6**.

**23 September 2026, late night. UIG-21 is done: `CLAUDE.md` is an index, and the rest loads only where it applies.** estiva-ui PR #84 (0.30.0, merge `e1df3e8`, tag `v0.30.0`, npm latest), peek PR #332 (`19ff88d`) and ship PR #177 (`c1fd8c2`), both deployed, and this record's PR. **What a session loads, measured with a real fresh session before and after** (extra tokens over an empty folder's 32,952): estiva-ui 194 lines and 8,260 tokens → 67 lines and 2,056 (−75%); Peek 351 lines (with the imported `UI-RULES.md`) and 9,892 → 69 and 1,566 (−84%); Ship 327 and 8,280 → 70 and 1,557 (−81%). **All 403 instructions were listed first**, each with one home, and Katerina ruled on the table before anything moved: kept in `CLAUDE.md`, moved to a `.claude/rules` file with `paths:`, said by a skill, held by a check, or deleted with a reason (out of date, said twice, history). **Rule files:** estiva-ui `components`, `stories-and-pages`, `tokens`, `release`; Peek `convex` (Jan's text word for word), `data-seam`, `on-device-ai`, `peek-ui`; Ship `ship-ui`, `wire-and-fold`. **Skills:** estiva-ui `port` and `verify`, Ship `verify`. **Proved:** a rule file loads only once a file its paths match is read, and also in a session started in the folder above the repos (`K:\Estiva`); a session there that opened a Convex file got Peek's Convex rules, and one that opened `package.json` did not. **Sentences became checks:** the token lint runs in the `gate` job in all three repos, so it blocks a merge; a new rule refuses a plain `tailwind-merge` and names `cn()`; `gates:status` gains `instructions()`, which fails when `CLAUDE.md` with its `@` imports reaches 200 lines or a rule file has no `paths:`. The skill now names both exception forms: the gate's, and the token lint's (A2). **Her other rulings:** the package skill otherwise untouched; Peek's prototype-era docs deleted (MIGRATION, PRODUCTION-PLAN, both Storybook plans, QA-PLAN, HOW-TO-RUN) and no reference to the prototype left, which stays as it is on disk and on GitHub; nothing points at the migration docs, and the old mentions in code comments and the changelog stay as history; Peek is "pixel-perfect to the package". **Leaf joins when UIG-11 makes it:** its `CLAUDE.md` starts in this shape, and `create-estiva-app`'s is already under 200 lines with no import (a test holds it). Found on the way and filed: PEE-27 (four Peek files read data around the seam), SHI-26 (three shared files differ from estiva-agent's copies), UIG-38 (the gate misses a hand-drawn `<table>` and an Escape on a field). This record also fixes a stale check: UIG-13's asked `ui:find` to name `--also peek --also ship`, which UIG-20 replaced with finding the neighbours. `gates:status` on the merged code: **UIG-21 9 of 9**; 27 done, none failing.

**23 September 2026, night. UIG-20 is done: in any repository, Claude searches every catalogue by itself before it builds.** This is Katerina's condition for having no shared Storybook (UIG-19). estiva-ui PR #81 (0.29.0, merge `349fbfc`, tag `v0.29.0`, npm latest), peek PR #326 (`f15253f`) and ship PR #176 (`c624d0d`), both deployed, and this record's PR. Katerina asked for one release with UIG-37, but 0.28.0 (UIG-37, below) was published before #81 merged. So the package has two releases, and **each app took both in one bump**: #326 and #176 carry UIG-37's app commits (peek `b002412`, ship `a1df8b2`) beside UIG-20's. The same PR stops the accessibility job running on pull requests: it runs on main only (her ruling, 23 September). The ticket was rewritten in Ship first with her six rulings and what had gone stale. **Estiva ID is left out by her ruling** ("isn't properly done yet"). It joins the search and the skill when it installs the package, with nothing to change here, and Leaf joins with UIG-11 in the same way.
...
**And from the folder that holds them:** `K:\Estiva` (Katerina's computer, not a repository) got its own loader and settings from `estiva-ui skill --root peek --out .`. It reads Peek's installed package. A fresh session started there, asked for "a thin bar for how many issues are done", opened the skill, ran the search inside Ship, and answered `ProgressBar variant="quiet"`, naming where Ship and Peek already draw one. It then asked where the bar should go rather than build.

`gates:status` on the merged code, from Katerina's checkouts: **UIG-20 18 of 18** (estiva-ui 9, Peek 4, Ship 5); UIG-37 2 of 2 in each app. Her Peek and Ship checkouts took 0.29.0 by swapping the package's folder for the published tarball, since nothing else changed. That left her running servers alone.

**23 September 2026, night. UIG-37 is done: the token lint and the gate stop refusing TypeScript's own `eslint-disable` comments.** estiva-ui PR #80 (0.28.0, merge `a9dc295`, tag `v0.28.0`). The apps took it inside 0.29.0, one PR each with UIG-20, by Katerina's choice: peek PR #326 (`f15253f`) and ship PR #176 (`c624d0d`). `gates:status` reads UIG-37 **6 of 6**: the package exports `tokenConfig` and its gate passes the comment, and in each app the comment passes the gate and every token config, and a token lint that runs alone is `tokenConfig`. Peek's `lint:tokens` reads 0 errors and the same 88 warnings as before, Ship's lint 0 and 25. This repo's ticket list had no UIG-37 row, so its status called the apps' rows "not one of the tickets"; the record PR adds it. **One correction:** the TypeScript comment was taken out of PR #324 before it merged, so Peek's main was never red; the fix is for the next one. What follows is the paragraph written while it was in review.
...
| ✅ **UIG-23** | estiva-ui PR #89 (0.32.0, merge `ed261a0`); peek PR #346 (`db5149a`), ship PR #183 (`ca367d9`). `estiva/no-handmade-empty-state`, in the apps and the package: a line drawn by hand where a list's rows would be, read by where it sits, not its words; a page `EmptyState` in a `ScrollArea` that cannot centre, followed across files. Package 2 and Peek 4 fixed, Ship 0, 0 escapes. `.verify-empty-scopes.mjs` retired. `gates:status` 5 of 5. |
| ✅ **UIG-24** | estiva-ui PR #91 (0.33.0, merge `0d2a708`); peek PR #349 (`85446a2`), ship PR #185 (`c61f1e2`). `estiva/no-native-title`, in the apps and the package: a `title` attribute is the browser's tooltip; `WithTooltip` is ours. 0 in all three today; on older commits the Folders timestamp, UIG-1's six and four more. Exclusions recorded as decisions. `gates:status` 5 of 5. |
| ✅ **UIG-25** | estiva-ui PR #93 (0.34.0, merge `ab59283`); peek PR #353 (`4689a00`), ship PR #188 (`3047e59`). `estiva/no-copied-look`, a warning: 4 look words covering 80%, against the parts' looks from the catalogue and the app's own. Fixed from photos: shared looks in the package, Peek's `MetaText`/`RepliesRow`/writing box, DateDivider on `SectionLabel`, frames as `Card`. The rest marked with her reasons: 0 warnings in all three. `gates:status` 6 of 6. |
| ⬜ **UIG-11** | Leaf, made by `create-estiva-app` from the current package. **Held by Katerina until the other tickets are done.** Needs from Jan: a sign-in client and address, the kinds it signs, relay access. `gates:status` 0 of 2. |
| ✅ **UIG-17** | peek PR #307 (merge `f13834f`) and estiva-ui PR #75: every reusable part of Peek has a usage page, and its Storybook follows her six rules. `gates:status` 2 of 2 (25 September: its three Storybook-map checks moved into the package's `estiva-ui check`, 0.35.0). |
...
| ⬜ **UIG-26** | The close. Waits for every other ticket and for UIG-11 (Leaf, held), her ruling of 25 September (§0). Its baseline is remade from 0.21.0 and 0.23.0 when it runs. `gates:status` 0 of 1. |
...
On 25 September, over installed copies of the three mains with peek #362 and ship #194 (`GATES_PEEK=../peek-fix GATES_SHIP=../ship-fix`), it ended:

```
✅ 31 done · 🚧 0 started · ⬜ 8 not started · ❔ 0 could not check
39 tickets. Owned by estiva-ui 34, peek 3, ship 2.
✅ Each ticket is owned by exactly one repo, and every repo agrees.
```

Each repo line above it says how far that checkout is behind main (0.35.0). A checkout that is behind reads old code: pull it first.
...
| ✅ done | UIG-10: estiva-ui PR #53, #54 and #55 (0.21.0) built the command (**UIG-10: building it**, below). Reopened 18 September: a made app is not connected to the relay (§0); the relay wiring in #65 (0.23.0). Closed 25 September, the connected-and-signed-in proof handed to UIG-11 |
...
| ✅ done | UIG-37: estiva-ui PR #80 (0.28.0); peek PR #326 and ship PR #176 took it inside 0.29.0 — the gates know TypeScript's rule names, and the token lint on its own is the package's `tokenConfig()` (§0) |
| ✅ done | UIG-20: estiva-ui PR #81 (0.29.0), peek PR #326, ship PR #176, all merged and deployed: the skill runs `estiva-ui find` unasked, in any repo and from `K:\Estiva`, across every Storybook; the apps took UIG-37's 0.28.0 in the same bump (§0) |
| ✅ done | UIG-21: estiva-ui PR #84 (0.30.0), peek PR #332, ship PR #177, all merged and deployed: `CLAUDE.md` is an index in all three repos, the rest loads only where it applies, and three sentences became checks (§0) |
| ✅ done | UIG-22: estiva-ui PR #86 (0.31.0), peek PR #338, ship PR #178, all merged and deployed: a column's header bar drawn by hand is refused, by its shape; `DialogShell` draws `ContainerHeader` (§0) |
| ✅ done | UIG-23: estiva-ui PR #89 (0.32.0), peek PR #346, ship PR #183, all merged and deployed: an empty state drawn by hand is refused, by where it sits; a page state that cannot centre too (§0) |
| ✅ done | UIG-24: estiva-ui PR #91 (0.33.0), peek PR #349, ship PR #185, all merged and deployed: the browser's `title` tooltip is refused, naming `WithTooltip` (§0) |
| ✅ done | UIG-25: estiva-ui PR #93 (0.34.0), peek PR #353, ship PR #188, all merged and deployed: a look copied by hand is warned about, never blocked; the copies she ruled on are fixed or carry their reason (§0) |
| ✅ done | The audit before UIG-26: estiva-ui PR #95 (0.35.0), peek PR #361, ship PR #193, merged; Ship deployed, Peek's deploy refused by GitHub billing. peek PR #362 and ship PR #194 wait for CI, which GitHub will not run until the billing is sorted (**The audit before UIG-26**, below) |
| ✅ done | Her rulings on both reviews (§0, 25 September evening): built as estiva-ui 0.36.0; the apps take it after |
| ⏸ next | Peek and Ship take 0.36.0 (name the escapes, switch `gate` to `estiva-gates ci`, the approved When not words); UIG-26 rewritten by name |
...
### The audit before UIG-26

**How it was checked, 24–25 September.** Four read-only checkers, one per part of the guide (the wall; the catalogue and the skill; the fingerprints and the starter; the paper trail), each quoting the guide's line and citing a file and line on `main`. Two more compared every ticket as first written (the backlog artifact, 12 September) with its Ship text today and this file. The findings that mattered most were re-read in the code before being reported. Nothing was changed while checking. **The review page, `https://claude.ai/artifact/7kqeDgG13Rcun9X6ccZxpD`, holds all 52**, each with the line it breaks, what is true, and a suggestion.

**Plan B, tactic by tactic.** Done: T10, T12, T15, T16, T5, T6. Partly: T11 (keyboard handling on one element passes, B7), T13 (`style` on a part is not read, B6), T14 (the hook skips the token rules, B4; it failed open, A2 — fixed), T22 (escapes are counted but not listed, reviewed or dated, B1; one marker silences every rule on its line, B3), T1 (behaviours are hand-listed and empty for app parts, B9; app links missing, B10), T2 (*When not* need not name an alternative, B8), T23 (Leaf not made; the count can rise, B2; the token lint was not required in a made app, B5 — fixed). Later by her ruling: T4, T7 (UIG-39).

**Fixed, no ruling needed (her word, 25 September):**

| | finding | fix |
|---|---|---|
| A1 | Ship moved out of `web/` (PER-19); estiva-ui's status, compare, rules and skills still read `web/`, so `gates:status` failed for Ship once pulled | estiva-ui #95: Ship read at its top folder, as Peek; `--app` kept for any app in a folder of its own |
| A2 | Ship's hook pointed at a top-folder install its checkout lacked; a hook that crashes exits 1, and Claude Code lets the write through; status passed any command containing "gates" | #95: `estiva-gates hook` refuses (exit 2) when it cannot run, and says to `npm ci`; status runs the committed hook on a raw `<button>` and passes only on exit 2, and fails a matcher that never fires for a Write. Proved: a hook with no ESLint in reach, before 1 and after 2 |
| A3 | Peek's Introduction said 49 reusable parts (the catalogue builds 41) and named a Huddles heading FOL-23 deleted; `HEADINGS` too; the checks ran only in `gates:status` | #95: the three checks in `estiva-ui check` (`src/registry/storymap.ts`), which every app's `gate` runs, each only when the app keeps that map; peek #361: the page and `HEADINGS` fixed, two "Seen in" lines that named deleted stories, Peek's copies deleted |
| A4 | `gates:status` read local checkouts; a copy seven commits behind reported UIG-25 as not started | #95: each repo line fetches and prints "N commits behind origin/main" |
| A5 | Peek's count check read a gitignored `registry.json` | #95: the check builds the catalogue in memory. Ship's UIG-18 class check had the same trap: ship #194 |
| B5 | A made app ran the token lint only in `check`, which no merge requires; nothing ran the app checks on a made app | #95: the token lint in the made `gate`; a test makes an app, links the package in, gives it a first commit and runs every app check on it for real (it failed on exactly this before the fix) |
| B16 | The made `CLAUDE.md` (and Peek's) listed three of the rules, written once | #95, peek #361: they point at the skill, whose list grows with the package |
| — | Peek's status read `TopicsPage.tsx` as its real page; FOL-23 deleted it, and three checks threw | peek #362: `FoldersPage.tsx`, which the gate passes with no error |
| E2, E5 | Stale lines in this file; four of her planning rulings written nowhere here | this record's PR: the lines below, and **Rulings on the guide's six questions** (§14) |
| E3 | Three tickets said the wrong thing | Ship, 25 September: UIG-11 says "0.23.0 or later", its step 4 no longer registers with the shared Storybook she ruled out, and it names her hold; a comment corrects the UIG-19 reminder on it. UIG-10 closed with a hand-off comment to UIG-11. UIG-23's closing comment corrected (`CommandLauncher.tsx:1464`, and the escapes that were not needed) |

**Measured after:** estiva-ui 1,477 unit tests pass; lint 0 errors and 140 warnings, the same as main; `registry:check` 87 entries, 172 links; `gates:status` over the merged mains (peek #362 and ship #194 included) **31 done, 8 not started (UIG-11, 26, 30, 31, 33, 34, 38, 39), 0 could not check**, 39 tickets.

**GitHub ran again from 25 September, evening.** Before that, **GitHub stopped running Actions for the private repos on 24 September, about 22:00 UTC**: "The job was not started because recent account payments have failed or your spending limit needs to be increased." Ship's deploy ran first; Peek's deploy (#361), and every job on peek #362 and ship #194, were refused. The merged Peek change draws nothing differently. Nothing can merge into any repo until the billing is sorted, because `gate` is required and cannot run.

**Her rulings on B, C, D and F came on 25 September, evening (§0).** Still hers: E1 (the Ship project description, which she pastes) and E4.

### UIG-25: building it

**24 September 2026.** Ship ticket UIG-25. Branch `gates/25-fp-copy` in all three repos; estiva-ui PR #93 → 0.34.0; peek PR #353, ship PR #188.

**How a look is read** (`src/eslint/looks-of.ts`, shared by the catalogue and the rule, so the two mean the same thing): every class list a file writes — a `className` or `…ClassName` through `cn()`, ternaries, `&&`, templates, a `const` in the file, a class map indexed by a key — and every string that is a class list on its own. A `&&` is read **off first**: a look is the part at rest with one option at a time, never every option at once; read the other way, `Card`'s look was one list of twenty words that matched anything. Only look words count. A class list written as a constant and used through `className` warns once, at the constant.

**The line, and how it was chosen.** Over the three repos with the census: 3 look words at 80% → 161 places, mostly common text styles; **4 at 80% → 62 places**, every real copy; 5 → missed the Card frame. Katerina: 4 and 80%. `MIN_SHARED` and `MIN_COVER` are exported from `@estiva-app/ui/eslint`.

**A correction she saw.** The first findings page came from a census that kept only each place's closest part before testing it; the built rule has no such shortcut and found 25 more places (N1–N10), shown to her with photos before anything was done about them.

**Every finding and its outcome** (letters as on her page):

| | what | outcome |
|---|---|---|
| A1, N1 | TextInput ≈ Textarea (16 words) ≈ Select; field sizes; SearchInput's shell ≈ ChipInput's | fixed: `FIELD_BOX/TEXT/DISABLED/FOCUS_RING/SIZE/SHELL_CLASSES` in `src/looks.ts`; Select's box marked (it drops the hover while held) |
| A2 | Menu's panel ≈ Tooltip ≈ a Divider story's fake menu | fixed: `FLOATING_SURFACE_CLASSES`; the story draws `MenuPanel` |
| A3 | CommandPalette's window ≈ DialogShell's | left alone, her word; both marked |
| A4 | Chip's words ≈ Reaction's count ≈ Peek's HighlightPill, TopicState | the package's two fixed (`CHIP_TEXT_CLASSES`); Peek's recorded, marked |
| A5 | the bare input: SearchInput, ChipInput, CommandPalette's search | fixed but the palette's (left alone, marked) |
| A6, N3 | AttachmentCard's name ≈ InputChip's label | recorded, marked |
| B1 | Peek's DateDivider ≈ SectionLabel | fixed: `SectionLabel tone="muted" truncate`; 22px → 24px, accepted |
| B2, N2 | Card's frame typed by hand: Ship's issues table, 14 story frames | fixed: `Card` (the table with no fill, her pick), `MenuPanel` for a floating one |
| B3, B4, B5, N9 | Ship's table headers ≈ Property's label; counts ≈ NavItem's; refs and an address ≈ Breadcrumb's id | recorded, marked (the header once, at `TH_CLASSES`) |
| C1 | Peek's time typed in nine places | fixed: `MetaText` (with a page and stories) |
| C2 | HuddleCard's replies row ≈ ConversationCard's | fixed: `RepliesRow` (with a page and stories) |
| C3 | the writing box, four times | fixed: `WRITING_BOX_CLASSES` |
| C4 | a 14px semibold title in three panels | recorded, marked |
| D1 | Peek's CommentAnchor ≈ Ship's BlockAnchorNote | recorded; the lint cannot see across apps |
| N4, N5, N7, N8, N10 | a round button (Lightbox, AttachmentCard); a row hover (NavItem, PersonTrigger, Peek's PersonRow); a status label; Ship's board column (a `<section>`, Card is a box) | recorded, marked |
| N6, W1, W2 | Card ≈ the palette's window; Reaction ≈ a field's box; Button ≈ Reaction | coincidences, left alone, marked |
| — | Peek's `WRITING_BOX_CLASSES` ≈ ChipInput's box; `PEEK_EDITOR_CLASSES` ≈ the bare input | Peek's own surfaces, recorded, marked |

**The ticket's acceptance, item by item.** Registered once, in the package's configs, a warning, reaching all three repos — yes. A hit does not fail `lint:rules`, CI or the hook — yes: no step passes `--max-warnings`, and the hook blocks on errors only. TextInput ≈ Textarea warns — yes, tested. Peek's `Avatar` does not warn — yes, tested (a wrapper named like its part). A part added to the catalogue is covered without touching the rule — yes, tested. All three repos, stories in, tests out — yes. **The ForeignObjectWidget case at its original commit — not met, and deliberately so.** At peek `3dc663b~1` it wrote `text-[10px] uppercase tracking-wide text-text-muted`; that day's SectionLabel wrote `signal:text-[10px] signal:uppercase signal:tracking-[0.14em] text-text-primary` (estiva-ui `743a4af`). They share two words and differ in colour, tracking and font: the same idea, not the same look, and no package part used those words. A rule that matched it would have to guess intent; this one compares what is written. The test holds the case in today's words instead.

**Arithmetic.** Warnings on the merged code: package 0 (20 escaped), Peek 0 (18), Ship 0 (5). An escape counts every element a mark sits above, including a few that already carried another rule's mark. 58 before-and-after photos identical; 2 changed, both hers (the date line, the table's hairline).

**Checks.** 16 rule tests; 1,467 package tests; Peek 148 tests in the touched parts, `tsc -b` clean in both apps; `gates:status` UIG-25 6 of 6, every other ticket unchanged.

### UIG-24: building it

**24 September 2026.** Ship ticket UIG-24. Branch `gates/24-fp-tooltip` in all three repos; estiva-ui PR #91 → 0.33.0; peek PR #349, ship PR #185.

**What it judges.** A JSX attribute named `title`: on a lowercase element, always; on a capitalised one, only when it is a part of the package — imported, re-exported, or an app's wrapper that spreads its props onto one — and the part's `title` is not its own. Every part but six hands a `title` it does not declare on to its element (`Button`, `IconButton`, `Card`, `Link`, `NavItem` and the others spread the rest of their props), so there it is the browser's box too.

**The exclusions, each a decision** (the ticket: "explicit and tested"):

| | what | decision | why |
|---|---|---|---|
| 1 | `<title>` inside an `<svg>` | never fires | an element, not an attribute: a label for a screen reader, not a tooltip. A fixture proves it |
| 2 | `<title>` in a document's head | never fires | the same: an element. No file writes one; a fixture covers it anyway |
| 3 | a part whose `title` is its own | passes | `CollapsibleSection`, `ConfirmDialog`, `ContainerHeader`, `DialogShell`, `ListColumn`, `SectionHeader`: the words of a heading, drawn on the page. `PARTS_WITH_A_TITLE`, held to `registry.json` by a test, so a part that gains a `title` of its own must be added on purpose |
| 4 | an app's own component | not judged | its `title` is its own business (Peek's `NewTopicBanner`); the element it draws is judged where it draws it |
| 5 | a story's `title` | never fires | `meta = { title: 'Group/Name' }` is an object key, not an attribute |
| 6 | a `title` on a non-interactive element | **not excluded** | the ticket allowed this only "if the count shows the rule is otherwise unusable". The count is 0, so the rule stays strict: a hover box on a `<span>` is still the browser's |

**The count.** The package 0, Peek 0, Ship 0 (every `title=` left in the three is one of the six parts' headings, a dialog's title, or an app component's own). **On older commits:** peek `3dc663b~1` — `ForeignConversationView.tsx:106` (the ticket's case), `MentionChip.tsx:53` (a raw nostr address on an unresolved mention) and `stories/SignalTheme.stories.tsx:57`, `:199`, `:209`; peek `d094006` — `Reference.tsx:120`, `:142`, `:168`; ship `6693025` — `Reference.tsx:91`, `:107`, `:139`. All real, all gone. No false positives.

**UIG-9's check, widened.** It read `src/eslint/index.ts` for the three app rules followed only by `no-handmade-*` rules; `no-native-title` would have turned it red, as the fourth rule did on 24 September (estiva-ui #88). It now takes any `no-*` rule after the three.

**Arithmetic.** 0 + 0 + 0 = **0**; fixed 0, escaped 0. The apps' `.gates-count.json` and the package's read `estiva/no-native-title: 0 errors, 0 warnings, 0 escapes`.

**Checks.** 23 rule cases; 1,451 package tests; `lint:rules` 0 errors in the package, Peek and Ship; `gates:status` UIG-24 5 of 5, every other ticket unchanged.

### UIG-23: building it

**24 September 2026.** Ship ticket UIG-23. Branch `gates/23-fp-empty` in all three repos; estiva-ui PR #89 → 0.32.0; peek PR #346, ship PR #183.

**UIG-1's table, reconciled on UIG-1's commits** (Peek `d094006`, Ship `6693025`, estiva-ui `89d27d6`), with the built rule:

| | where | the text | the rule | why |
|---|---|---|---|---|
| ✅ | `CommandLauncher.tsx:1464` | "Nothing here matches what any app offers to make…" | silent | a reply to a request (Make asked the model; nothing matched), not a list with no rows — the same kind as the two ❓ below. Katerina: leave. Today it is the palette's `FieldLine` |
| ✅ | `CommandLauncher.tsx:1716` | "Nothing for that here." | **fires** | under `rows.length === 0 &&`, in a box |
| ✅ | ship `Board.tsx:61` | "Nothing here" | **fires** | the other side of the column's `.map` |
| ✅ | ship `ForeignObject.tsx:206` | "Nothing in it yet." | **fires** | under `children?.length === 0 &&` |
| ✅ | `ReactionPicker.stories.tsx:100` | "Nothing here yet." | **fires** | the other side of the `.map` |
| ❌ | `CommandLauncher.tsx:1498` | "Nothing is created until you submit it." | silent ✓ | a hint beside a button; it stands in for nothing |
| ❌ | `FolderContentsView.tsx:101` | "…this folder has no folder state yet." | silent ✓ | under `folder.source === 'channel' &&`, which asks nothing about a list |
| ❓ | `CommandLauncher.tsx:1514` | "No answer came back…" | silent | **Decision: not an empty state.** A failure after a request; the other side is an answer with its sources listed inside it, not the list. Katerina: leave |
| ❓ | `CommandLauncher.tsx:1622` | "No draft came back…" | silent | **Decision: not an empty state**, the same. Katerina: leave |
| ❓ | `ReadStatePanel.tsx:65` | "No comparison yet — it needs one relay fetch." | silent | **Decision: not an empty state.** A debug line under `if (!report)`, waiting for a report, not standing in for a list. Katerina: leave |

The same run found two more UIG-1 missed by its wording, both Peek dialogs (`AddToOpenWorkDialog`, `MoveFileDialog`); they are two of today's four. **The launcher escapes the ticket asked for are not needed:** the launcher draws on the package's palette now, and its lines are `FieldLine`s, a part.

**The four wrong-scope states, at peek `7f22e5e~1`:** `FoldersPage.tsx:259` reports `FolderContentsView` twice (its `ChildRows`, lines 337 and 339, two components down) and `:321` reports `ForeignConversationView` twice (lines 64 and 78). All four.

**`.verify-empty-scopes.mjs`, retired.** Run on the same trees (a copy that takes a folder): at `7f22e5e~1` it marked the same two call sites the rule reports (the rule names the four states); on today's Peek and Ship it finds nothing, as the rule does. It could not run on Ship without editing (its folder was Peek's), read no stories, looked only at the first capitalised tag within 220 characters of `<ScrollArea`, and in the package marked `ListColumn` wrapping `ErrorBoundary`, a false alarm (the tag was in the next function; `ListColumn` draws its broken message outside the `ScrollArea` on purpose). Deleted from Katerina's Peek checkout, where it lived untracked.

**Every remaining `page` state was checked**, in all three repos, stories in: each fills a whole empty column or page (Peek's `FoldersPage` "Select a folder.", `ObjectPage`, `FilePage`, `TopicsPage`, the DM and topic views' own centred boxes; Ship's `IssuePage`, `ProjectPage`, `ProjectsView`, `App`; the package's `ErrorBoundary` and `AppShell` stories). None fires.

**What it cannot see yet:** a class component (it follows function components), and a package part other than `EmptyState` that draws a page state — `ErrorBoundary` placed inside a `ScrollArea`. Neither exists today.

**Arithmetic.** Package 2 + Peek 4 + Ship 0 = **6**; fixed 6, escaped 0. Wrong-scope 0 + 0 + 0. The apps' `.gates-count.json` and the package's read `estiva/no-handmade-empty-state: 0 errors, 0 warnings, 0 escapes`.

**Checks.** 34 rule cases; 1,428 package tests; `lint:rules` 0 errors in the package, Peek and Ship; Peek's dialog test, `tsc -b` and `lint:tokens`; `gates:status` UIG-23 5 of 5, every other ticket unchanged. After-photos in Peek's Storybook: each of the four dialog lines level with its title (0px).
...
#### Three things it found (the first was fixed on the way, below)
...
| every ticket, again | **Where a repo's gate files are, after UIG-32.** Peek: `eslint.gates.config.js`, `eslint.tokens.config.js`, `scripts/gates-checks.mjs`, `.claude/settings.json`, `docs/GATES-DEBT.md`. Ship: the same, at its top folder since PER-19 (24 September; until then all in `web/`, with `--app web`). A made app: `appFiles()` in `src/gates/create-app.ts`. Nothing else is a gate file; if you find one, it is a copy. |
...
| rule | repo | count | samples | message | verdict |
|---|---|---|---|---|---|
| **no-handmade-header** | estiva-ui | 1 → **0** | `src/DialogShell.tsx:117`, which owns the shape | `A panel header drawn by hand. Use SectionHeader.` (UIG-1's draft; the built rule names `ContainerHeader`, and UIG-22's count is in §0) | **on** |
...
| **no-handmade-empty-state** | estiva-ui | 0 → **2 → 0** | `src/FilePicker.stories.tsx:25` · `src/ReactionPicker.stories.tsx:101` (UIG-23's count; fixed) | `A line saying there is nothing is an EmptyState. Use EmptyState from @estiva-app/ui: scope="section" inside a section, scope="page" for a whole page.` | **on** |
| | **peek** | 7 → **4 → 0** | `components/AddToOpenWorkDialog.tsx:64` · `MoveFileDialog.tsx:76` · `MoveUnderDialog.tsx:81` · `StartTopicDialog.tsx:106` (UIG-23's count; fixed) | same | |
| | **ship** | 2 → **0** | `Board.tsx:61` and `ForeignObject.tsx:206` became `EmptyState` in UIG-14 | same | |

> **Recounted by UIG-23, 24 September**, with the built rule, which reads where a line sits and not its words. UIG-1's 7 + 2 were a wording pass: of Peek's 7, five were the launcher's (now the palette's `FieldLine`, a part, which the rule does not judge), and two it must stay silent on. See §0, **UIG-23: building it**.
| **no-native-title** | estiva-ui | **0** | none found | ``title` is the browser's tooltip, not ours. Wrap it in `WithTooltip` from @estiva-app/ui, or use `IconButton`'s `tooltip`: its page (Primitives/Tooltip) says when it shows and how.`` | **on** |
| | **peek** | 3 → **0** | `components/ui/Reference.tsx` ×3, converted in UIG-14 | same | |
| | **ship** | 3 → **0** | `components/ui/Reference.tsx` ×3, converted in UIG-14 | same | |

> **Recounted by UIG-24, 24 September**, with the built rule: 0 in all three repos. UIG-1's six are found on UIG-1's commits, with four it did not count (a mention chip, three story lines). See §0, **UIG-24: building it**.
| **no-copied-look** (was no-copied-class-list) | estiva-ui | 14 → **16 → 0** warnings, 20 escaped | fixed: TextInput ≈ Textarea ≈ Select, Menu ≈ Tooltip, Chip ≈ Reaction, SearchInput ≈ ChipInput, 14 story frames; marked: see UIG-25 | `This look is also written in X. Write it once in src/looks.ts` · `This class list is FIELD_BOX_CLASSES, typed again. Use it.` | **warn** |
| | **peek** | 28 → **12 → 0** warnings, 18 escaped | fixed: the time in nine places (`MetaText`), the replies row (`RepliesRow`), the writing box, DateDivider (`SectionLabel`); marked: see UIG-25 | `This class list is X's look, typed again. Use X from @estiva-app/ui.` | |
| | **ship** | 2 → **11 → 0** warnings, 5 escaped | fixed: the issues table's frame (`Card`); marked: see UIG-25 | same | |
...
Nothing in this document is now waiting on a ticket that does not exist. *(Not true by 25 September: the audit before UIG-26 found thirteen things with no ticket and no place on UIG-26's list — C5's chip and thread line just above among them. They are D1–D13 on its review page, waiting on Katerina for a home; see §0.)*
...
### Rulings on the guide's six questions

The guide ends with six questions (§10, "What I need from you"). Her answers were
given while planning, on 12 September, and lived only in the roadmap artifact and
session notes until the audit before UIG-26 found them written nowhere here.

| | question | her answer |
|---|---|---|
| Q1 | A, B or C? | **Plan B**, "Know, then Refuse". |
| Q2 | See the rule list with counts before building it? | **Yes**: UIG-1, and her verdicts above. |
| Q3 | Chromatic, or the free version? | **Not decided.** It matters only for visual regression, a next-phase tactic. |
| Q4 | How public is public? | **"Open-sourcing is far off, Leaf is very close."** So `create-app` moved from last to phase 2; `DESIGN.md` and `llms.txt` moved to a later phase (UIG-39); the catalogue is built for us, with `estiva-ui find` instead of public docs files. |
| Q5 | Does Jan get a say in the rule list? | **The lint blocks Jan's pull requests too** (her yes), and the committed hook reaches him because he works with Claude Code in these repos. Jan was told what changes, the hook and how to escape a line on 16 September, the day the wall went on (UIG-6). No record shows him the rule list before it became a gate, or the rules added since (the audit's B14, open). |
| Q6 | When does Leaf start? | **"Very close"**, which set the order. On 18 September she held Leaf (UIG-11) until the other tickets are done; it is still held. |
...
| added later | UIG-31, UIG-33 to UIG-39 | UIG-35 to UIG-37 are done; UIG-31, 33, 34, 38 and 39 are open, and where each goes when the project closes is Katerina's to rule (the audit's D13). |
...
| UIG-13 | The registry widens to Peek's and Ship's | estiva-ui | peek, ship | in estiva-ui: `buildAppRegistry` and `findInRegistries` exported; `registry.json` is the package's own at schema 2, with no app entry (the package is public); `ui:find` names no neighbour, so it finds them all (UIG-20); create-app writes `ui:find`, `registry:check` in job `gate` and ignores `registry.json`. In each app (appChecks, confirmed by UIG-13, 18 September): `ui:find` and `registry:check` run the package's commands, CI's job `gate` runs `registry:check`, the app's folder ignores `registry.json`, and `h.catalogue` builds it: every part described and sorted |
...
| UIG-17 | Usage rules — Peek's own components | peek | | every reusable part keeps the usage-page contract (the package's `h.contract`), and CI's gate runs `registry:check`, which since 0.35.0 holds Peek's Storybook map too |
...
| UIG-20 | The Claude skill | estiva-ui | peek, ship | `skill/estiva-ui.md` ships and step one is the search; `find` finds its neighbours; the package's own loader reads the text; `check` runs `checkLoader`; the `gate` job runs `skill:check`; the hook runs `searchFirst`; the starter writes the loader; `docs/GATES-SKILL.md` reconciles 542 = 111 + 431; in each app, the loader reads the installed package, `.claude/settings.json` allows the search, `ui:find` names no neighbour (Ship's top folder has one too), and `gate` runs a check that refuses a drifted loader (9 checks in the package, 4 or 5 per app) |
| UIG-21 | CLAUDE.md becomes an index | estiva-ui | peek, ship | `CLAUDE.md` with its imports under 200 lines and every `.claude/rules` file scoped by `paths:` (`instructions()`); CI's job gate runs the token lint; a plain tailwind-merge is refused |
| UIG-22 | Fingerprint — hand-made header row | estiva-ui | peek, ship | probe: a padded row at the top of a pane names ContainerHeader, in every app |
...
| UIG-37 | The gates refuse a TypeScript eslint-disable comment | estiva-ui | peek, ship | a TypeScript directive is no error in the gate or the token lint, in each repo; a token lint on its own is the package's `tokenConfig` (row added 25 September; the ticket was built 23 September) |
| UIG-38 | The gate misses a hand-drawn table and a hand-written Escape | estiva-ui | | first guess: `no-raw-element` maps `<table>` (added 25 September, the audit before UIG-26) |
| UIG-39 | DESIGN.md and llms.txt (later phase) | estiva-ui | | first guess: both files ship in the package (added 25 September) |
...
| | |
|---|---|
| tickets | **39** (25 September; `gates:status` reads the same) |
| owned by estiva-ui | **34** — UIG-1, 2, 5 to 16, 19 to 28, 30 to 39 |
...
| **S2** | The registry is a data source | A versioned schema with stable field names, read by machines first and rendered second. One per repo — never merged into a committed index, and an app's never committed at all (Katerina, 18 September, UIG-13). | The MCP server, the duplicate scan and any public docs file read it and nothing else. A Markdown generator would need three parsers. | UIG-12, UIG-13 |
...
| peek | since UIG-32 (17 September) | `docs/GATES-DEBT.md`, created by peek PR #245 |
...
On 13 September there are **0** debt lists and **0** entries. On 16 September, after UIG-4 and UIG-5: **2** debt lists, Ship's and the package's, with **0** entries between them. Peek had none then; UIG-32 gave it one on 17 September (§0).
...
| ⬜ | UIG-3, UIG-5 | The roadmap puts a `docs/GATES-DEBT.md` in every repo, and UIG-20 reads "all repos' `GATES-DEBT.md`". Only UIG-4 creates one. | **Closed.** The package's came with UIG-5, Peek's with UIG-32 (17 September); UIG-20 read all three. |
