...
Nothing in CI photographs anything (D70, point 3), so the proof is yours.

Every non-empty diff is either a ruling (named in the PR) or a defect. "It looks the same" is not a result.

## 1. Photograph the stories the change reaches (D70)

The scripts are in `K:/Estiva/migration docs/photo-check/`; its `README.md` has the details.
They load `playwright-core` from `K:/Estiva/peek` and drive installed Chrome.

1. **Which stories.** Only the stories whose files reach the changed component through their imports, never the whole Storybook:
   `node stories-using.mjs <repo root> <storybook url> "<Name\b" > ids.txt`
   Repo root: this worktree for the package (`http://localhost:6008`); `K:/Estiva/<worktree>` for Peek; `K:/Estiva/<worktree>/web` for Ship.
2. **Before**, on the unchanged code, **once**: `node shoot.mjs <storybook url> <dir> <theme> @ids.txt`.
   Four tabs at once; each photo waits for the story's play script.
3. **After**: the same command into another folder.
4. **Compare**: `node compare.mjs <before dir> <after dir>`. It lists only the files that differ. A channel off by 8 or less is noise.
5. **Look**: `node crop.mjs <a.png> <b.png> <x> <y> <w> <h> <out.png> [scale]` puts the two crops side by side, magnified; the compare prints the box. A difference is explained only when its crop has been looked at.
...
The older `shots-themed.mjs` in `../peek/.verify-shots` shoots this Storybook in a named theme, but every story, one at a time. D70 replaced it with `shoot.mjs`.
...
Some defects show in no package canvas. Findings 38 and 39 (2026-09-08): a control re-created on a state change, a required field named with its asterisk, a disabled trigger that opens — none visible in Storybook, all three in an app's first hour. The app's first hour is part of the work (D19). Read every app failure as a possible package defect before touching the app's test.
